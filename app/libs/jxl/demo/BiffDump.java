...
public class BiffDump
{
  ...
  /**
   * Constructor.  Occasionally invoked as part of the debugging process
   * to display portions of the defragged excel file
   *
   * @param data the data
   * @param os the output stream
   * @exception IOException 
   * @exception BiffException
   */
  public BiffDump(byte[] data, OutputStream os) 
    throws IOException, BiffException
  {
    writer = new BufferedWriter(new OutputStreamWriter(os));
  ...
  private void buildNameHash()
  {
    ...
    recordNames.put(Type.ALRUNS, "ALRUNS");
    ...
    throws IOException
  {
    ...
    if (code == Type.FORMAT.value)
    {
      buf.append(" (0x");
      buf.append(Integer.toHexString(formatIndex));
      buf.append(")");
      formatIndex++;
    }
